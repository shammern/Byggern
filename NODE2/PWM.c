...
void PWM_init(){
	PIOB->PIO_ABSR |= PIO_PB12B_PWMH0 | PIO_PB13B_PWMH1 ;  //Enables PWM module on PIO controller B function B pin PB12/13.
	
	PIOB->PIO_PDR |= PIO_PB12B_PWMH0 | PIO_PB13B_PWMH1; //Disable PIO controller for controlling PWM pins
	
	PWM->PWM_DIS = PWM_DIS_CHID0;	//Disables PWM on channel 0
	...
	PWM->PWM_CH_NUM[0].PWM_CPRD = PWM_CPRD_CPRD(CPRDB); //Sets PWM Periode for clock B
	PWM->PWM_CH_NUM[1].PWM_CPRD = PWM_CPRD_CPRD(CPRDA); //Sets PWM Periode for clock A
	
	
	PWM->PWM_ENA = PWM_DIS_CHID0;	//Enables PWM on channel 0
...
void PWM_set_duty(float duty_cycle, uint8_t channel){
	...
	int D = 0;
	D = (int) (PWM->PWM_CH_NUM[channel].PWM_CPRD*(1-duty_cycle)); //calculates D based on periode stored in CPRD register
	PWM->PWM_CH_NUM[channel].PWM_CDTY = PWM_CDTY_CDTY(D);	//Sets duty cycle
}
